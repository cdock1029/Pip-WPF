...
{
    public static int CompareTerm(this Treasury t, Treasury other)
    {
        if (t.MaturityDate is null || other.MaturityDate is null)
            return t.SecurityType.CompareTo((int)other.SecurityType);

        int thisSpan = t.MaturityDate.Value.DayNumber - t.IssueDate!.Value.DayNumber;
        int otherSpan = other.MaturityDate.Value.DayNumber - other.IssueDate!.Value.DayNumber;
