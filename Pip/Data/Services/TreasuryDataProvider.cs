...
namespace Pip.UI.Data.Services;
...
public class TreasuryDataProvider : ITreasuryDataProvider
{
    ...
    public ValueTask<Treasury?> LookupTreasuryAsync(string cusip, DateOnly? issueDate, CancellationToken ct)
    {
        ...
        (string cusip, DateOnly? issueDate) key = (cusip, issueDate);
        ...
        Task<Treasury?> task = _cache.GetOrCreateAsync(key, _ => _client.GetFromJsonAsync<Treasury>(path, ct));
    ...
    {
        Investment? investment = await _dbContext.Investments.FindAsync(id).ConfigureAwait(false);
